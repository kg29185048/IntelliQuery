...
def ask_database(user_query: str) -> str:
    ...
    """
    if db is None:
        return "❌ Database Error: Invalid MONGO_URI. Please provide a database name in the URI."
        
    print(f"\n[MCP] Claude triggered pipeline with: {user_query}", file=sys.stderr)
    
    # Phase 1: Extract Intent
    intent_result = extract_intent_pipeline(db, user_query=user_query, history=None)
    if "error" in intent_result:
        print(f"[MCP] Intent Extraction failed: {intent_result['error']}", file=sys.stderr)
        return f"❌ Pipeline Error:\n{intent_result['error']}"
        
    extracted_intent = intent_result.get("extracted_intent", {})
    
    # Phase 2: Run your compiled LangGraph pipeline with the extracted intent
    pipeline_state = run_pipeline(db, user_query=user_query, history=None, confirmed_intent=extracted_intent)
